 ...
 *  - When `allDay` is true, DTSTART/DTEND use VALUE=DATE per RFC 5545 ┬º3.6.1.
...
/** Fold a single logical line per RFC 5545 ┬º3.1 (75-octet limit, CRLF + space). */
...
export function convert(input: ConvertInput): ConvertOutput {
  ...
  if (input.url?.trim()) {
    // URL is a calendar-address-style property; do not text-escape (RFC 5545 ┬º3.8.4.6).
