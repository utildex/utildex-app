 ...
 *
 *     ÔöîÔöÇÔöÇ seconds      (optional, 0-59)
 *     Ôöé ÔöîÔöÇÔöÇ minutes    (0-59)
 *     Ôöé Ôöé ÔöîÔöÇÔöÇ hours    (0-23)
 *     Ôöé Ôöé Ôöé ÔöîÔöÇÔöÇ day-of-month (1-31)
 *     Ôöé Ôöé Ôöé Ôöé ÔöîÔöÇÔöÇ month (1-12 or JAN-DEC)
 *     Ôöé Ôöé Ôöé Ôöé Ôöé ÔöîÔöÇÔöÇ day-of-week (0-7, SUN-SAT; 0 and 7 = Sunday)
 *     Ôöé Ôöé Ôöé Ôöé Ôöé Ôöé ÔöîÔöÇÔöÇ year (optional, 1970-2099)
 *     * * * * * * *
 *
 * Supported syntax:
 *  - `*`            → wildcard
...
function describeNumericList(values: number[], pad: (n: number) => string): string {
  ...
  while (i < values.length) {
    ...
    else groups.push(`${pad(values[i])}ÔÇô${pad(values[j])}`);
    i = j + 1;
  }
  return groups.join(', ');
}
...
export function describe(parts: CronParts, labels: DescribeLabels): string {
  ...
  if (m.isWildcard && h.isWildcard) {
    ...
    if (times.length <= 6) {
      ...
      phrases.push(labels.atTime(`${times[0]}ÔÇª${times[times.length - 1]} (${times.length}├ù)`));
    }
  }
