  ...
  template: `
    <div class="space-y-8">
      <div class="flex items-center gap-3">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ t.map()['TITLE'] }}</h1>
        <span class="px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-mono text-slate-500">{{ categories().length }}</span>
      </div>
