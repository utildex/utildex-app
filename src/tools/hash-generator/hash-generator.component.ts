    ...
    <ng-template #mainContent>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h3 class="font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2 uppercase tracking-wide text-xs">
              <span class="material-symbols-outlined text-lg">settings</span> Options
            </h3>
...
export class HashGeneratorComponent {
  ...
  private async computeHash(data: Uint8Array, algo: HashAlgorithm) {
    ...
    if (algo === 'MD5') {
      ...
      // Use Web Crypto API for SHA algorithms - ensure we have a proper ArrayBuffer
    ...
    // Stocker le hash brut en minuscules - le computed hashResult() appliquera la transformation
