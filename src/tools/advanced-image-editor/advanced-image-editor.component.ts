...
type FilterPreset = 'none' | 'sequoia' | 'vivid' | 'matte' | 'vintage' | 'bw' | 'cool' | 'warm';
...
@Component({
  ...
  template: `
    @if (!isWidget()) {
      ...
      <div
        class="flex h-full min-h-0 flex-col rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800"
      >
        ...
        <!-- Header (required) -->
        <div
          class="flex items-center justify-between gap-2 border-b border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-900/40"
        >
          <div class="flex min-w-0 items-center gap-2">
            <span class="material-symbols-outlined text-primary">tune</span>
            <div class="min-w-0">
              <div
                class="truncate text-xs font-bold tracking-wide text-slate-700 uppercase dark:text-white"
              >
                {{ t.map()['TITLE'] }}
              </div>
              @if (viewMode() === 'expanded') {
                <div class="truncate text-[11px] text-slate-500 dark:text-slate-300">
          ...
          <div class="flex items-center gap-1">
            <button
              type="button"
              class="rounded-lg border border-slate-200 px-2 py-1 text-[11px] text-slate-700 hover:bg-white dark:border-slate-700 dark:text-white dark:hover:bg-slate-800"
        ...
        <div class="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_auto] gap-2 p-2">
          @if (images().length === 0) {
            <button
              type="button"
              class="flex h-full w-full flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-700/30"
              (click)="openWidgetFilePicker()"
            ...
            >
              ...
              >
              <div class="px-2 text-center text-xs text-slate-600 dark:text-slate-200">
                {{ t.map()['WIDGET_EMPTY_HINT'] }}
              </div>
            </button>
          } @else {
            <!-- Main editor area -->
            <div class="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_140px] gap-2">
              <!-- Live preview -->
              <div
                class="relative min-h-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/30"
              >
                <canvas #widgetPreviewCanvasEl class="absolute inset-0 h-full w-full"></canvas>

                <div
                  class="absolute top-2 left-2 rounded bg-white/80 px-2 py-1 text-[11px] text-slate-800 dark:bg-slate-900/70 dark:text-white"
                >
                ...
                <div class="absolute top-2 right-2 flex gap-1">
                  <button
                    type="button"
                    class="rounded bg-white/80 px-2 py-1 text-[11px] text-slate-800 hover:opacity-90 disabled:opacity-40 dark:bg-slate-900/70 dark:text-white"
                    (click)="widgetPrev()"
              ...
              <div
                class="flex min-h-0 flex-col rounded-lg border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-800"
              >
                <!-- Tab bar -->
                <div
                  class="grid grid-cols-3 gap-1 rounded-lg border border-slate-200 bg-slate-100 p-1 dark:border-slate-700 dark:bg-slate-900/40"
                >
                  <button
                    type="button"
                    class="rounded-md py-1 text-[11px] font-medium transition-colors"
                ...
                <!-- Panel content -->
                <div class="mt-2 min-h-0 flex-1 space-y-2 overflow-auto">
                  @if (widgetTab() === 'edit') {
                  ...
                  @if (widgetTab() === 'filters') {
                    ...
                    <label
                      class="flex flex-col gap-1 text-[11px] text-slate-600 dark:text-slate-200"
                    >
                      {{ t.map()['FILTERS'] }}
                      <select
                        class="rounded-lg border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-700 dark:border-slate-700 dark:bg-slate-900/30 dark:text-white"
                  ...
                  @if (widgetTab() === 'export') {
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-1">
                      <button
                        type="button"
                        class="rounded-lg border border-slate-200 px-2 py-2 text-[11px] text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                        (click)="export(false)"
                      ...
                      <button
                        type="button"
                        class="bg-primary rounded-lg px-2 py-2 text-[11px] text-white hover:opacity-90"
                        (click)="export(true)"
                        [disabled]="exportJob().running"
                      >
            ...
            <div class="grid shrink-0 grid-cols-2 gap-2">
              <button
                type="button"
                class="rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                (click)="openWidgetFilePicker()"
              >
              ...
              <a
                class="rounded-lg bg-slate-900 px-3 py-2 text-center text-xs text-white hover:opacity-90 dark:bg-slate-700"
                href="#/tools/advanced-image-editor"
    ...
    <ng-template #mainContent>
      <div
        class="min-h-[520px] rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4 dark:border-slate-700 dark:bg-slate-800"
      >
        <!-- Top bar -->
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <button
              class="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
              (click)="undoGlobal()"
            ...
            <button
              class="bg-primary flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-white hover:opacity-90"
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-[240px_1fr_320px]">
          <!-- Left: Queue -->
          <div
            class="flex min-h-[260px] flex-col rounded-xl border border-slate-200 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-900/40"
          ...
          >
            ...
            @if (images().length === 0) {
              <button
                type="button"
                class="flex w-full flex-1 items-center justify-center rounded-lg border border-dashed border-slate-300 p-3 text-center hover:bg-white dark:border-slate-700 dark:hover:bg-slate-800"
                ...
                @for (img of images(); track img.id) {
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="truncate text-sm font-medium text-slate-700 dark:text-white">
                        {{ img.name }}
                      </div>
                      <div
                        class="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-300"
                      >
                        @if (img.status === 'exporting') {
                          <span>{{ t.map()['STATUS_EXPORTING'] }}</span>
                        }
                        @if (img.status === 'done') {
                          <span>{{ t.map()['STATUS_DONE'] }}</span>
                        }
                        @if (img.status === 'error') {
            ...
            <div class="flex gap-2 pt-2">
              <button
                class="flex-1 rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-white dark:border-slate-700 dark:text-white dark:hover:bg-slate-800"
          ...
          <!-- Center: Preview -->
          <div
            class="flex min-h-[320px] flex-col rounded-xl border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-800"
          >
            <div class="flex items-center justify-between pb-2">
              <div class="text-sm font-semibold text-slate-700 dark:text-white">
                {{ t.map()['PREVIEW'] }}
              ...
              <div class="flex items-center gap-2">
                <button
                  class="rounded-lg border border-slate-200 px-2 py-1 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                  (click)="fitPreview()"
                >
                  {{ t.map()['FIT'] }}
                </button>
            ...
            <div
              class="relative flex-1 overflow-hidden rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/30"
            >
              <canvas
                #previewCanvasEl
              ...
              @if (cropUi().enabled && cropUi().dragging) {
                <div
                  class="absolute top-2 left-2 rounded bg-white/80 px-2 py-1 text-[11px] text-slate-800 dark:bg-slate-900/80 dark:text-white"
                >
                  {{ t.map()['CROP'] }}: {{ cropUi().x.toFixed(0) }},{{ cropUi().y.toFixed(0) }} —
                  {{ cropUi().w.toFixed(0) }}×{{ cropUi().h.toFixed(0) }}
                </div>
              }
          ...
          >
            <div class="flex items-center justify-between pb-2">
              ...
              <div class="flex items-center gap-2">
                <label class="text-xs text-slate-600 dark:text-slate-200">{{
                  t.map()['APPLY_TO']
                }}</label>
                <select
                  class="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-900/30 dark:text-white"
                ...
                <button
                  class="flex-1 px-2 py-2 rounded-lg text-xs border
                    ...
                    activeTab() === tab.id
                      ...
                      : 'border-slate-200 dark:border-slate-700 text-slate-700 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-700'
                  }}"
            ...
            <div class="flex-1 overflow-auto pr-1">
              @if (!activeImage()) {
                <div class="p-3 text-sm text-slate-500 dark:text-slate-300">
                  {{ t.map()['EMPTY_QUEUE'] }}
                </div>
              } @else {
                @if (activeTab() === 'adjust') {
                  <div class="space-y-3 p-1">
                    <div class="grid grid-cols-2 gap-2">
                      <button
                        class="rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                        (click)="undoImage()"
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['BRIGHTNESS'] }}: {{ workingRecipe().brightness }}</label
                      >
                      <input
                        type="range"
                        min="-100"
                        ...
                        [ngModel]="workingRecipe().brightness"
                        (ngModelChange)="updateWorking('brightness', $event)"
                        (change)="commitWorking('Brightness')"
                        class="accent-primary w-full"
                      />
                    </div>
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['CONTRAST'] }}: {{ workingRecipe().contrast }}</label
                      >
                      <input
                        type="range"
                        min="-100"
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['SATURATION'] }}: {{ workingRecipe().saturation }}</label
                      >
                      <input
                        type="range"
                        min="-100"
                        ...
                        [ngModel]="workingRecipe().saturation"
                        (ngModelChange)="updateWorking('saturation', $event)"
                        (change)="commitWorking('Saturation')"
                        class="accent-primary w-full"
                      />
                    </div>
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['EXPOSURE'] }}: {{ workingRecipe().exposure }}</label
                      >
                      <input
                        type="range"
                        min="-100"
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['GAMMA'] }}: {{ workingRecipe().gamma.toFixed(2) }}</label
                      >
                      <input
                        type="range"
                        min="0.2"
                        ...
                        [ngModel]="workingRecipe().gamma"
                        (ngModelChange)="updateWorking('gamma', $event)"
                        (change)="commitWorking('Gamma')"
                        class="accent-primary w-full"
                      />
                    </div>

                    <button
                      class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                      (click)="resetAllEdits()"
                    >
                      {{ t.map()['RESET_ALL'] }}
                    </button>
                ...
                @if (activeTab() === 'crop') {
                  <div class="space-y-3 p-1">
                    <div class="grid grid-cols-2 gap-2">
                      ...
                      <button
                        class="bg-primary rounded-lg px-2 py-2 text-xs text-white hover:opacity-90"
                    ...
                    <div class="rounded-lg border border-slate-200 p-2 dark:border-slate-700">
                      <div class="flex items-center justify-between">
                        <div class="text-xs font-semibold text-slate-700 dark:text-white">
                          {{ t.map()['CROP'] }}
                        </div>
                        <label
                          class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-200"
                        >
                          <input
                            type="checkbox"
                            class="accent-primary"
                            [checked]="workingRecipe().cropEnabled"
                            (change)="
                              updateWorking('cropEnabled', $any($event.target).checked);
                              commitWorking('Crop enabled')
                            "
                          />
                          on
                        </label>
                      ...
                      <div class="mt-2 grid grid-cols-2 gap-2">
                        <div>
                          <label class="text-[11px] text-slate-600 dark:text-slate-200">{{
                            t.map()['ASPECT']
                          }}</label>
                          <select
                            class="w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-900/30 dark:text-white"
                        ...
                        <div class="flex items-end">
                          <button
                            class="w-full rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                            (click)="resetCrop()"
                          >
                            {{ t.map()['RESET_CROP'] }}
                          </button>
                      ...
                      <div
                        class="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-700 dark:text-white"
                      >
                        <label class="flex flex-col gap-1">
                          X
                          <input
                            class="rounded-lg border border-slate-200 bg-white px-2 py-2 dark:border-slate-700 dark:bg-slate-900/30"
                            type="number"
                            step="0.01"
                            min="0"
                            max="1"
                            ...
                            (ngModelChange)="
                              updateWorking('cropX', clamp($event, 0, 1)); commitWorking('Crop X')
                            "
                          />
                        </label>
                        <label class="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            (ngModelChange)="
                              updateWorking('cropY', clamp($event, 0, 1)); commitWorking('Crop Y')
                            "
                          />
                        </label>
                        <label class="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            (ngModelChange)="
                              updateWorking('cropW', clamp($event, 0.01, 1));
                              commitWorking('Crop W')
                            "
                          />
                        </label>
                        <label class="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            (ngModelChange)="
                              updateWorking('cropH', clamp($event, 0.01, 1));
                              commitWorking('Crop H')
                            "
                          />
                        </label>
                      </div>
                      ...
                      <div class="mt-2 grid grid-cols-2 gap-2">
                        <button
                          class="flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                          (click)="rotate(-90)"
                        >
                      ...
                      @if (cropSequential().active) {
                        <div class="border-primary bg-primary/10 mt-3 rounded-lg border p-2">
                          ...
                          <div class="mt-2 grid grid-cols-3 gap-2">
                            <button
                              class="rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-white dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                              (click)="seqBack()"
                          ...
                          <button
                            class="mt-2 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs text-slate-700 hover:bg-white dark:border-slate-700 dark:text-white dark:hover:bg-slate-700"
                            (click)="seqDone()"
                          >
                            {{ t.map()['DONE'] }}
                          </button>
                ...
                @if (activeTab() === 'filters') {
                  <div class="space-y-3 p-1">
                    ...
                    <div class="space-y-2">
                      <label class="text-xs text-slate-600 dark:text-slate-200"
                        >{{ t.map()['INTENSITY'] }}: {{ workingRecipe().presetIntensity }}</label
                      >
                      <input
                        type="range"
                        min="0"
                        ...
                        [ngModel]="workingRecipe().presetIntensity"
                        (ngModelChange)="updateWorking('presetIntensity', $event)"
                        (change)="commitWorking('Preset intensity')"
                        class="accent-primary w-full"
                      />
                    </div>

                    <div
                      class="space-y-3 rounded-lg border border-slate-200 p-2 dark:border-slate-700"
                    >
                      <div class="text-xs font-semibold text-slate-700 dark:text-white">
                        {{ t.map()['BLUR'] }} / {{ t.map()['SHARPEN'] }}
                      </div>

                      <div class="space-y-2">
                        <label class="text-xs text-slate-600 dark:text-slate-200"
                          >{{ t.map()['BLUR_RADIUS'] }}: {{ workingRecipe().blurRadius }}</label
                        >
                        <input
                          type="range"
                          min="0"
                      ...
                      <div class="space-y-2">
                        <label class="text-xs text-slate-600 dark:text-slate-200"
                          >{{ t.map()['SHARPEN_AMOUNT'] }}:
                          {{ workingRecipe().sharpenAmount }}</label
                        >
                        <input
                          type="range"
                          min="0"
                ...
                @if (activeTab() === 'export') {
                  <div class="space-y-3 p-1">
                    <div class="grid grid-cols-2 gap-2">
                      <label class="flex flex-col gap-1 text-xs text-slate-600 dark:text-slate-200">
                        {{ t.map()['FORMAT'] }}
                        <select
                          class="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-900/30 dark:text-white"
                          [ngModel]="exportSettings().format"
                          (ngModelChange)="setExportFormat($event)"
                        >
                          <option value="image/png">PNG</option>
                    ...
                    @if (exportSettings().resizeMode === 'exact') {
                      <div class="grid grid-cols-2 gap-2">
                        <label
                          class="flex flex-col gap-1 text-xs text-slate-600 dark:text-slate-200"
                        >
                          {{ t.map()['WIDTH'] }}
                          <input
                            type="number"
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-1">
                      ...
                      -->
                      <button
                        class="bg-primary rounded-lg px-3 py-2 text-sm text-white hover:opacity-90"
                        (click)="export(true)"
                    ...
                    @if (exportJob().running) {
                      <div
                        class="mt-2 rounded-lg border border-slate-200 p-2 dark:border-slate-700"
                      >
                        <div class="flex items-center justify-between">
                        ...
                        <div
                          class="mt-2 h-2 w-full overflow-hidden rounded bg-slate-200 dark:bg-slate-700"
                        >
                          <div
...
export class AdvancedImageEditorComponent {
  ...
  private scopeIdsForSequentialCrop(): string[] {
    ...
    if (scope === 'selected')
      return Array.from(this.selectedIds()).filter((id) => this.images().some((i) => i.id === id));
    // "this"
  ...
  private previewRectToNormalizedCrop(
    x: number,
    y: number,
    w: number,
    h: number,
  ...
  setWidgetPreset(preset: FilterPreset) {
    ...
    this.workingRecipe.update((r) => {
      const nextIntensity = preset === 'none' ? 0 : r.presetIntensity > 0 ? r.presetIntensity : 70;
  ...
  private commitOperation(
    ...
    label: string,
    imageIds: string[],
    buildAfter: (img: ImageItem) => Recipe,
  ) {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        for (let ch = 0; ch < 3; ch++) {
          const val =
            5 * s[c + ch] - 1 * s[up + ch] - 1 * s[dn + ch] - 1 * s[lf + ch] - 1 * s[rt + ch];
  ...
  async export(all: boolean) {
    ...
    const snapshot = targets.map((i) => ({
      id: i.id,
      name: i.name,
      file: i.file,
      recipe: deepCloneRecipe(i.recipe),
      version: i.version,
    }));
    ...
    this.images.update((list) =>
      list.map((img) =>
        ids.includes(img.id) ? { ...img, status: 'exporting', error: undefined } : img,
      ),
    );
    ...
    try {
      for (let i = 0; i < snapshot.length; i++) {
        ...
        try {
          ...
          this.images.update((list) =>
            list.map((img) =>
              img.id === s.id ? { ...img, status: 'error', error: errorMessage } : img,
            ),
          );
        }
  ...
  private async renderForExport(
    ...
    return await new Promise((resolve, reject) => {
      outCanvas.toBlob((b) => (b ? resolve(b) : reject(new Error('toBlob failed'))), mime, quality);
    });
  }
