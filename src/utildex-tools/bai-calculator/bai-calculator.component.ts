...
@Component({
  selector: 'app-bai-calculator',
  standalone: true,
  imports: [CommonModule, FormsModule, ToolLayoutComponent, ActionBarComponent],
  providers: [provideTranslation({ en: () => en, fr: () => fr, es: () => es, zh: () => zh })],
  template: `
    <app-tool-layout toolId="bai-calculator">
      <div
        class="bai-card mx-auto flex w-full max-w-4xl flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6 dark:border-slate-700 dark:bg-slate-800"
      >
        <!-- Top row: hip + height (single line on md+) -->
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 md:items-start">
          <div>
            ...
            <div class="flex items-stretch gap-1.5">
              ...
              <select
                [ngModel]="hipUnit()"
                (ngModelChange)="setHipUnit($event)"
                [attr.aria-label]="t.map()['UNIT_TOGGLE_HIP']"
        ...
        >
          ...
          @if (axisVisible() && gradientPercent() !== null) {
            <div class="mt-3" role="img" [attr.aria-label]="axisAriaLabel()">
              <div class="bai-gradient relative h-3 w-full overflow-visible rounded-full">
                <div
      ...
      @if (methodologyOpen()) {
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="bai-modal-title"
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <h2 id="bai-modal-title" class="text-lg font-bold text-slate-900 dark:text-slate-100">
            ...
            <div class="mt-3 space-y-3 text-sm text-slate-600 dark:text-slate-300">
              ...
              <ul class="space-y-2 text-xs">
                <li>
                  <a
                    href="https://doi.org/10.1038/oby.2011.38"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-primary underline-offset-2 hover:underline"
                  >
                    {{ t.map()['REF_BERGMAN_2011'] }}
                  </a>
                </li>
                <li>
                  <a
                    href="https://doi.org/10.1038/oby.2012.81"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-primary underline-offset-2 hover:underline"
                  >
                    {{ t.map()['REF_FREEDMAN_2012'] }}
                  </a>
                </li>
                <li class="text-slate-600 dark:text-slate-300">
                  {{ t.map()['REF_LOPEZ_2012'] }}
                </li>
              </ul>
            </div>
          </div>
  ...
  styles: [
    `
      ...
      /* Color gradient: blue (underweight) -> green (normal) -> yellow -> red (obese) */
      .bai-gradient {
      ...
      .profile-defaults-badge {
        animation: bai-badge-pulse 2s ease-in-out infinite;
      }

      @keyframes bai-badge-pulse {
        0%,
        100% {
          opacity: 1;
        }
...
export class BaiCalculatorComponent {
  ...
  constructor() {
    this.persistence.storage(this.hipInput, 'bai-hip-input', {
      type: 'object',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.heightInput, 'bai-height-input', {
      type: 'object',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.hipUnit, 'bai-hip-unit', {
      type: 'string',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.heightUnit, 'bai-height-unit', {
      type: 'string',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.sex, 'bai-sex', { type: 'string', strategy: 'hybrid' });
    this.persistence.storage(this.age, 'bai-age', { type: 'number', strategy: 'hybrid' });
  }
  ...
  /** Position 0..100 of marker. Anchors gradient to current band thresholds. */
  gradientPercent = computed<number | null>(() => {
    ...
    if (v < t.underweightMax) return clamp((v / Math.max(t.underweightMax, 0.001)) * 25);
    if (v < t.normalMax)
      return clamp(
        25 + ((v - t.underweightMax) / Math.max(t.normalMax - t.underweightMax, 0.001)) * 25,
      );
    if (v < t.overweightMax)
      return clamp(50 + ((v - t.normalMax) / Math.max(t.overweightMax - t.normalMax, 0.001)) * 25);
    return clamp(75 + ((v - t.overweightMax) / Math.max(t.overweightMax * 0.25, 0.001)) * 25);
  });
  ...
  setHipUnit(value: LengthUnit) {
    this.hipUnit.set(value);
  }
