...
import {
  ...
  type Tier,
  type WeightUnit,
} from './body-fat-deurenberg.kernel';
...
@Component({
  selector: 'app-body-fat-deurenberg',
  standalone: true,
  imports: [CommonModule, FormsModule, ToolLayoutComponent, ActionBarComponent],
  providers: [provideTranslation({ en: () => en, fr: () => fr, es: () => es, zh: () => zh })],
  template: `
    <app-tool-layout toolId="body-fat-deurenberg">
      <div
        class="bf-card mx-auto flex w-full max-w-4xl flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6 dark:border-slate-700 dark:bg-slate-800"
      >
        <!-- Top row: weight + height -->
        ...
        >
          ...
          @if (axisVisible() && gradientPercent() !== null) {
            <div class="mt-3" role="img" [attr.aria-label]="axisAriaLabel()">
              <div class="bf-gradient relative h-3 w-full overflow-visible rounded-full">
                <div
          ...
          @if (hasInputs() && results().bmi !== null) {
            <div
              class="mt-4 grid grid-cols-3 gap-2 border-t border-slate-200 pt-3 dark:border-slate-700"
            >
              ...
              <div class="text-center">
                <p class="text-[10px] font-bold tracking-wider text-slate-500 uppercase">
                  {{ t.map()['FAT_MASS_LABEL'] }}
                </p>
                <p class="text-base font-semibold text-slate-700 tabular-nums dark:text-slate-200">
                  {{ formatMassDisplay(results().fatMassKg) }}
                </p>
              </div>
              <div class="text-center">
                <p class="text-[10px] font-bold tracking-wider text-slate-500 uppercase">
                  {{ t.map()['LEAN_MASS_LABEL'] }}
                </p>
                <p class="text-base font-semibold text-slate-700 tabular-nums dark:text-slate-200">
                  {{ formatMassDisplay(results().leanMassKg) }}
                </p>
              </div>
            </div>
          }
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <h2 id="bf-modal-title" class="text-lg font-bold text-slate-900 dark:text-slate-100">
            ...
            <div class="mt-3 space-y-3 text-sm text-slate-600 dark:text-slate-300">
              ...
              >
                <li>{{ t.map()['METHOD_FORMULA_ADULT'] }}</li>
                <li>{{ t.map()['METHOD_FORMULA_CHILD'] }}</li>
              </ul>
              ...
              <ul class="space-y-2 text-xs">
                <li>
                  <a
                    href="https://doi.org/10.1079/BJN19910073"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-primary underline-offset-2 hover:underline"
                  >
                    {{ t.map()['REF_DEURENBERG_1991'] }}
                  </a>
                </li>
                <li>
                  <a
                    href="https://doi.org/10.1038/sj.ijo.0800741"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-primary underline-offset-2 hover:underline"
                  >
                    {{ t.map()['REF_DEURENBERG_1998'] }}
                  </a>
                </li>
                <li>
                  <a
                    href="https://doi.org/10.1093/ajcn/72.3.694"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-primary underline-offset-2 hover:underline"
                  >
                    {{ t.map()['REF_GALLAGHER_2000'] }}
                  </a>
                </li>
              </ul>
            </div>
  ...
  styles: [
    `
      ...
      /* Color gradient: blue (essential) -> emerald -> lime -> yellow -> red (obese) */
      .bf-gradient {
...
export class BodyFatDeurenbergComponent {
  ...
  constructor() {
    this.persistence.storage(this.weightInput, 'bfd-weight-input', {
      type: 'object',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.heightInput, 'bfd-height-input', {
      type: 'object',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.weightUnit, 'bfd-weight-unit', {
      type: 'string',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.heightUnit, 'bfd-height-unit', {
      type: 'string',
      strategy: 'hybrid',
    });
    this.persistence.storage(this.sex, 'bfd-sex', { type: 'string', strategy: 'hybrid' });
    this.persistence.storage(this.age, 'bfd-age', { type: 'number', strategy: 'hybrid' });
  }
  ...
  primaryTier = computed<Tier | null>(() => {
    ...
    if (this.resolved().adjustments.veryYoungBlocked) return null;
    const v = this.results().bodyFatPercent;
    if (v === null) return null;
    return classify(v, this.resolved().thresholds);
  });

  axisVisible = computed(() => this.hasInputs() && !this.resolved().adjustments.veryYoungBlocked);
  ...
  gradientPercent = computed<number | null>(() => {
    ...
    if (v < t.essentialMax) return clamp((v / Math.max(t.essentialMax, 0.001)) * 20);
    if (v < t.athleticMax)
      return clamp(
        20 + ((v - t.essentialMax) / Math.max(t.athleticMax - t.essentialMax, 0.001)) * 20,
      );
    if (v < t.fitnessMax)
      return clamp(40 + ((v - t.athleticMax) / Math.max(t.fitnessMax - t.athleticMax, 0.001)) * 20);
    if (v < t.averageMax)
      return clamp(60 + ((v - t.fitnessMax) / Math.max(t.averageMax - t.fitnessMax, 0.001)) * 20);
    return clamp(80 + ((v - t.averageMax) / Math.max(t.averageMax * 0.4, 0.001)) * 20);
  });
  ...
  interpretation = computed<{ summary: string; notes: string[] } | null>(() => {
    ...
    if (adj.veryYoungBlocked) {
      return { summary: map['NOTE_VERY_YOUNG_BLOCKED'], notes: [] };
    }
    if (adj.pediatricFormula) notes.push(map['NOTE_PEDIATRIC_FORMULA']);
    if (adj.olderAdultNote) notes.push(map['NOTE_OLDER_ADULT']);
    if (adj.ethnicityCaveat) notes.push(map['NOTE_ETHNICITY']);
  ...
  exportText = computed(() => {
    ...
    const lines = [
      ...
      '',
      `${map['PRIMARY_RESULT_LABEL']}: ${fmt(r.bodyFatPercent)}%`,
      `${map['INTERPRETATION_HEADING']}: ${this.tierLabel()}`,
      `${map['BMI_LABEL']}: ${fmt(r.bmi)}`,
  ...
  tierStyleFor(tier: Tier | null): TierStyle {
    return TIER_STYLE[tier ?? 'fitness'];
  }
  ...
  setHeightUnit(value: LengthUnit) {
