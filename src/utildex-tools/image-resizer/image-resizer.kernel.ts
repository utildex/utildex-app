...
export async function run(
  ...
  try {
    const sourceMimeType = input.sourceMimeType ?? 'image/png';
    const sourceUrl = `data:${sourceMimeType};base64,${input.sourceBytes}`;
    ...
    const imageBytes = await blobToBase64(resized);
    return { success: true, imageBytes, error: null };
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Image resize failed';
